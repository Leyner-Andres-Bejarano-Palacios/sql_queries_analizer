...
In phase #1 (method __fn_find_referenced_tables) we generate a list of every table in the cluster (the rule does not apply on tables created by the user). we check if the name of those tables is present in the query
