...
class WithLimit(QueryTestAB):
    ...
    def __init__(self, query, logger, properties):
        super(WithLimit, self).__init__(query,logger, properties)
        ...
        self.__vWithsLimit = int(properties['withslimit'])
    @property
    def vName(self):
        return self._vName
    @property
    ...
    def fn_execute(self):
        ...
        for matchNum, match in enumerate(matches, start=1):
            vWithQuantity +=   len(match.groups())
        if vWithQuantity > self.__vWithsLimit:
            self._vLogger.info(self.LIMIT_EXCEEDED.format(self._vName))
            self._vResult = False
            return False,vWithQuantity,"more than "+str(self.__vWithsLimit)+" with words found"
        self._vLogger.info(self.TEST_SUCCEEDED.format(self._vName))
        self._vResult = True
        return True,vWithQuantity,"more than "+str(self.__vWithsLimit)+" with words found"
