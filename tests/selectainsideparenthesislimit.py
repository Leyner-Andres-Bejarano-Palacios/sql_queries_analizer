...
#-------------------------------------------------------------------------
"""
class SelectaInsideParenthesisLimit(QueryTestAB):
    ...
    def __init__(self, query, logger, properties):
        super(SelectaInsideParenthesisLimit, self).__init__(query,logger, properties)
        ...
        self.__vSelectaInsideParenthesisLimit = int(properties['selectainsideparenthesislimit'])
        self.__vSelectaInsideParenthesisRegex = properties['selectinsideparenthesisregex']
    @property
    def vName(self):
        return self._vName
    @property
    ...
    def fn_execute(self):
        ...
        vSelectaInsideParenthesisQuantity = 0
        pattern = self.__vSelectaInsideParenthesisRegex
        matches = re.finditer(pattern, self._vQuery, flags=re.MULTILINE)
        for matchNum, match in enumerate(matches, start=1):
            vSelectaInsideParenthesisQuantity +=   len(match.groups())
        if vSelectaInsideParenthesisQuantity > self.__vSelectaInsideParenthesisLimit:
            self._vLogger.info(self.LIMIT_EXCEEDED.format(self._vName))
            self._vResult = False
            return False,vSelectaInsideParenthesisQuantity,"in (select) found"
        self._vLogger.info(self.TEST_SUCCEEDED.format(self._vName))
        self._vResult = True
        return True,vSelectaInsideParenthesisQuantity,"in (select) found"